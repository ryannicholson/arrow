...
public abstract class FlightSQLProducer implements FlightProducer, AutoCloseable {
  ...
  public FlightInfo getFlightInfo(CallContext context, FlightDescriptor descriptor) {
    final Any command = descriptor.getCommand();
  ...
  public Runnable acceptPut(CallContext context, FlightStream flightStream, StreamListener<PutResult> ackStream) {
    final Any command = flightStream.getDescriptor().getCommand();
  ...
  public void doAction(CallContext context, Action action, StreamListener<Result> listener) {
    ...
    if (action.getType().equals(FLIGHT_SQL_GETSQLCAPABILITIES.getType())) {
      ...
      getCatalogs(FlightSQLUtils.unpackOrThrow(action.getBody(), ActionGetCatalogsRequest.class),
              context, listener);

    } else if (action.getType().equals(FLIGHT_SQL_GETSCHEMAS.getType())) {
      getSchemas(FlightSQLUtils.unpackOrThrow(action.getBody(), ActionGetSchemasRequest.class),
              context, listener);

    } else if (action.getType().equals(FLIGHT_SQL_GETTABLES.getType())) {
      getTables(FlightSQLUtils.unpackOrThrow(action.getBody(), ActionGetTablesRequest.class),
              context, listener);
      ...
      getPreparedStatement(FlightSQLUtils.unpackOrThrow(action.getBody(), ActionGetPreparedStatementRequest.class),
              context, listener);

    } else if (action.getType().equals(FLIGHT_SQL_CLOSEPREPAREDSTATEMENT.getType())) {
      closePreparedStatement(FlightSQLUtils.unpackOrThrow(action.getBody(), ActionClosePreparedStatementRequest.class),
              context, listener);
    }
  }
